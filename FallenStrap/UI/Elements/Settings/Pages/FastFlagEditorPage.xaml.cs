...
using FallenStrap.UI.Elements.Dialogs;
...
namespace FallenStrap.UI.Elements.Settings.Pages
{
    ...
    public partial class FastFlagEditorPage
    {
        ...
        private void ImportJSON(string json)
        {
            ...
            // support the alternate "array of objects" format some community tools export:
            // [ { "name": "FFlagX", "value": "True", "type": "bool", "enabled": true }, ... ]
            if (json.StartsWith('['))
            {
                try
                {
                    ...
                    var entries = JsonSerializer.Deserialize<List<NamedFastFlagEntry>>(json, arrayOptions);
                    ...
                    if (entries is not null)
                    {
                        ...
                        foreach (var entry in entries)
                        {
                            ...
                            // skip flags explicitly marked disabled in the source file
                            if (!entry.Enabled)
                                continue;
                        ...
                        json = JsonSerializer.Serialize(converted);
                    }
                }
                catch
                {
                    // not a valid array format, fall through to normal object parsing below,
                    // which will show the standard "invalid JSON" error if it also fails
                }
            }
        ...
        private void ImportFileButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                Filter = "FastFlags file (*.json;*.txt)|*.json;*.txt|All files (*.*)|*.*",
            ...
            {
                Frontend.ShowMessageBox($"No se pudo importar el archivo: {ex.Message}", MessageBoxImage.Error);
            }
        }
        ...
        private void ExportFileButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                Frontend.ShowMessageBox("FastFlags exportados correctamente.", MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                Frontend.ShowMessageBox($"No se pudo exportar el archivo: {ex.Message}", MessageBoxImage.Error);
            }
        }
        ...
        {
            var result = Frontend.ShowMessageBox(
                "Esto eliminara TODOS los FastFlags configurados. Esta accion no se puede deshacer. Continuar?",
                MessageBoxImage.Warning,
                MessageBoxButton.YesNo
            );
    ...
    // represents entries in the community "array of objects" FastFlag export format:
    // [ { "name": "...", "value": "...", "type": "bool|int|string", "enabled": true } ]
    internal class NamedFastFlagEntry
    {
